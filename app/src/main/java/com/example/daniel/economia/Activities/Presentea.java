...
public class Presentea extends AppCompatActivity {
    EditText anual;
    EditText number;
    EditText interest;
    TextView result;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_presentea);
    ...
    public void calculaPresentea(View v){
        if(anual.getText().toString().equals("hi") || number.getText().toString().equals("") || interest.getText().toString().equals("")){
            ...
            double anualp = Double.parseDouble(anual.getText().toString());
            double numero = Double.parseDouble(number.getText().toString());
            double interes = Double.parseDouble(interest.getText().toString());
            double res = anualp * ((Math.pow(1 + (interes / 100), numero) - 1) / ((interes / 100) * Math.pow((1 + (interes / 100)), numero)));
            result.setText("" + res);
        }
    }
}
